...
********************************************************************************************************************//**
* \file Ld::CppElseOperatorTranslator class.
***********************************************************************************************************************/
...
namespace Ld {
    CppElseOperatorTranslator::~CppElseOperatorTranslator() {}


    QString CppElseOperatorTranslator::elementName() const {
        return ElseOperatorElement::elementName;
    }


    bool CppElseOperatorTranslator::threadImplementation(ElementPointer element, CppCodeGenerationEngine& engine) {
        bool success = true;

        ElementPointer operation = element->child(0);
        CppContext& context = engine.context();

        if (!operation.isNull()) {
            context(element) << "else {\n";
    ...
    bool CppElseOperatorTranslator::methodDefinitions(ElementPointer element, CppCodeGenerationEngine& engine) {
        bool success;
