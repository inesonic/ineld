...
* \file
*
* This file implements the \ref Ld::CppLnFunctionTranslationEngine class.
***********************************************************************************************************************/
...
namespace Ld {
    const CppLnFunctionTranslationEngine CppLnFunctionTranslationEngine::instance;

    CppLnFunctionTranslationEngine::CppLnFunctionTranslationEngine() {}


    CppLnFunctionTranslationEngine::~CppLnFunctionTranslationEngine() {}


    bool CppLnFunctionTranslationEngine::identifyDependenciesAndExplicitTypes(
    ...
    bool CppLnFunctionTranslationEngine::implementation(
            const FunctionData&             functionData,
            QSharedPointer<FunctionElement> element,
            CppCodeGenerationEngine&        engine
        ) const {
        QString            functionMethod            = functionData.internalName();
        ElementPointerList lnParameters              = element->parameterElements();
        ElementPointerList parameters                = lnParameters;
        bool               requiresPerThreadInstance = functionData.requiresPerThreadInstance();

        if (lnParameters.size() == 1) {
            ElementPointer child         = lnParameters.first();
            QString        childTypeName = child->typeName();

            if (childTypeName == Ld::FunctionElement::elementName) {
                QString text1 = child->text(0);
                QString text2 = child->text(1);

                if (text1 == QString(QChar(0x0393)) && text2.isEmpty()) {
                    functionMethod = "M::lnGamma";
                    parameters     = child->children();
                }
            } else if (childTypeName == Ld::FactorialOperatorElement::elementName) {
                functionMethod = "M::lnFactorial";
                parameters     = child->children();
