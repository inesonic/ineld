...
* \file
*
* This file implements the \ref Ld::HtmlFunctionPlaceholderElementTranslator class.
***********************************************************************************************************************/
...
namespace Ld {
    HtmlFunctionPlaceholderElementTranslator::~HtmlFunctionPlaceholderElementTranslator() {}


    QString HtmlFunctionPlaceholderElementTranslator::elementName() const {
        return Ld::FunctionPlaceholderElement::elementName;
    }


    bool HtmlFunctionPlaceholderElementTranslator::body(ElementPointer element, HtmlCodeGenerationEngine& engine) {
        bool success;
        ...
        if (mathMode == HtmlCodeGenerationEngine::MathMode::MATHML         ||
            ...
            context.writeStartElement("menclose");
            context.writeAttribute("mathcolor", "gray");
            context.writeAttribute("notation", "box");

            context.writeStartElement("mo");
            context.writeAttribute("mathcolor", "gray");
            context.writeAttribute("mathvariant", "italic");
            context.writeCharacters(fieldString);
            context.writeEndElement();

            context.writeEndElement();
        } else if (mathMode == HtmlCodeGenerationEngine::MathMode::MATHJAX_LATEX) {
            context.writeCharacters("\\color{gray}{\\boxed{" + tr("?") + "}}");
        }
