...
********************************************************************************************************************//**
* \file Ld::CppLogicalConditionalOperatorTranslator class.
***********************************************************************************************************************/
...
namespace Ld {
    CppLogicalConditionalOperatorTranslator::~CppLogicalConditionalOperatorTranslator() {}


    QString CppLogicalConditionalOperatorTranslator::elementName() const {
        return LogicalConditionalOperatorElement::elementName;
    }


    bool CppLogicalConditionalOperatorTranslator::threadImplementation(
        ...
        if (!parent.isNull() && !child0.isNull() && !child1.isNull()) {
            unsigned long  childIndex = parent->indexOfChild(element);
            if ((parent->parentRequires(childIndex) & Capabilities::expressions).isNotEmpty()) {
                ...
                // We're using this operator as a relational operator.
    ...
    bool CppLogicalConditionalOperatorTranslator::methodDefinitions(
            ElementPointer           element,
            CppCodeGenerationEngine& engine
        ) {
        bool success;
